...
import login_bolivariano_selenium
...
PROCESS_NAME_MAX_LEN = 300  # nvarchar(300) en AutomationLog.processName


def escribirLog(mensaje, id_ejecucion, estado, accion):
    """Escribe un log en la BD (tabla AutomationLog). Colapsa el mensaje a una
    sola línea y lo trunca al ancho real de la columna processName (nvarchar
    300) para evitar el error 2628 'Los datos binarios o de la cadena se
    truncan' cuando el mensaje es largo (ej. un stacktrace de Selenium)."""
    mensaje_una_linea = " ".join(mensaje.split())[:PROCESS_NAME_MAX_LEN]
    texto_limpio = mensaje_una_linea.replace("'", "''")
    sql = f"""
...
def descargarMovimientosSelenium(id_ejecucion):
    """
    Ejecuta el RPA de Selenium: login (usuario + contraseña + OTP por
    correo), clic en la cuenta y descarga del archivo TXT de movimientos,
    dejándolo directamente en RUTAS_CONFIG['bolivariano'] (la carpeta que
    luego lee obtenerArchivos()).

    Devuelve True si la descarga se completó, False si falló (el error ya
    queda registrado en logs y en BD, con una captura de pantalla del punto
    exacto donde se detuvo en RUTA_CAPTURAS/BOLIVARIANO_CAPTURAS).
    """
    ...
    if not usuario or not password:
        LogManager.escribir_log(
            "ERROR", "Faltan BOLIVARIANO_USERNAME/BOLIVARIANO_PASSWORD en el .env")
        escribirLog("Faltan credenciales BOLIVARIANO_USERNAME/BOLIVARIANO_PASSWORD",
                    id_ejecucion, "Error", "Descarga RPA")
        return False

    ruta_descargas = RUTAS_CONFIG['bolivariano']
    os.makedirs(ruta_descargas, exist_ok=True)

    # OJO: headless=True es detectado por el banco ("Navegador no soportado").
    # Este proceso debe correr bajo Xvfb (DISPLAY=:99, ver bashPichincha.sh
    # como referencia) con BOLIVARIANO_HEADLESS=false (valor por defecto).
    headless = os.environ.get("BOLIVARIANO_HEADLESS", "false").strip().lower() not in ("false", "0", "no")

    LogManager.escribir_log(
        "INFO", f"🌐 Iniciando RPA de descarga (headless={headless})...")
    driver = None
    try:
        driver = login_bolivariano_selenium.crear_driver(
            headless=headless, ruta_descargas=ruta_descargas)
        ruta_archivo = login_bolivariano_selenium.login_y_descargar_movimientos(
            driver, usuario, password, ruta_descargas)
        LogManager.escribir_log(
            "SUCCESS", f"✅ Archivo descargado por RPA: {ruta_archivo}")
        escribirLog(f"Archivo descargado: {os.path.basename(ruta_archivo)}",
                    id_ejecucion, "Information", "Descarga RPA")
        return True
    except Exception as e:
        LogManager.escribir_log(
            "ERROR", f"Error en el RPA de descarga: {str(e)}")
        escribirLog(f"Error en el RPA de descarga: {str(e)}",
                    id_ejecucion, "Error", "Descarga RPA")
        return False
    finally:
        if driver:
...
def main():
    ...
    try:
        ...
        # Ejecutar el RPA (login + OTP + clic en cuenta + descarga TXT)
        # antes de buscar archivos: sin este paso nunca se abre el navegador.
        # Si falla, se corta acá (vía except) en vez de seguir como si nada:
        # de lo contrario "0 archivos para procesar" se reporta como éxito
        # aunque la descarga real haya fallado.
        if not descargarMovimientosSelenium(id_ejecucion):
            raise Exception(
                "Falló la descarga de movimientos vía RPA (Selenium); "
                "revisar capturas en BOLIVARIANO_CAPTURAS y logs.")
